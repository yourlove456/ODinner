...
<c:forEach var="file" items="${fileList}">
   <form action="up/upload/updateDir" method="post" enctype="multipart/form-data" class="form-horizontal">
      ...
      <c:if test="${(filetype eq '.JPG') or (filetype eq 'JPEG') or (filetype eq '.PNG') or (filetype eq '.GIF')}">
      		<li><div class="slide_image" style = "display:flex; justify-content: center; align-items: center;"><img src='<c:url value="/up/img/${file.fileId}"/>' width="1000px" height="600px" align="middle"></div><li></c:if>
      <c:if test="${!((filetype eq '.JPG') or (filetype eq 'JPEG') or (filetype eq '.PNG') or (filetype eq '.GIF'))}">
      		<a href='<c:url value="/up/pds/${file.fileId}"/>'>${file.fileName}</a><br></c:if>
 	</form>
</c:forEach>
</div>
<br><br><br><br>
